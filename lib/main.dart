...
class DownloadsPageState extends State<DownloadsPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("تحميلات R_Hunter", style: TextStyle(fontWeight: FontWeight.bold)), backgroundColor: Colors.transparent, centerTitle: true),
      body: Column(
        children: [
          ...
          Expanded(
            child: _isLoading 
              ...
              : !_hasPermission 
                  ...
                  : _files.isEmpty ? _buildEmptyState() : _buildFilesList(),
          ),
        ],
      ),
    );
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 20), 
          const Text("النظام يمنع الوصول للملفات!", style: TextStyle(fontSize: 18, color: Colors.white, fontWeight: FontWeight.bold)),
          const SizedBox(height: 10),
          const Text("اضغط هنا لكسر الحماية وعرض التحميلات", style: TextStyle(color: AppColors.textMain)),
          const SizedBox(height: 20),
          ElevatedButton(
            ...
            child: const Text("منح الصلاحية بالقوة", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold))
          )
        ]
      )
    );
  ...
  Widget _buildEmptyState() {
    return const Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.folder_open_rounded, size: 100, color: AppColors.border), SizedBox(height: 20), Text("مجلد R_Hunter فارغ", style: TextStyle(fontSize: 18, color: AppColors.textMain))]));
  }
